...
#2016 actual multilate data (pulled from OECD.stat front end since our DAC2A extract doesn't have it)
...
tableau.prelim.2017.plus <- tableau.prelim.2017 %>%
  ...
  # but only keep the columns that we need to fill in DAC2A since we have DAC1 data already from the API
...
tableau.combined <- tableau.base.dac1 %>%
  # Because 2017 is already in the dac1 data via the API, we can left join on the additional fields from Dac2a
